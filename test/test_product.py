...
class TestProductBlueprint(BaseTestCase):
    ...
    def test_create_product_invalid_product_name(self):
        with self.client:
            ...
            self.assertTrue(
                ...
                in create_product_data["errors"]["product_name"]
            )
            self.assertEqual(
                create_product_data["message"], "Input payload validation failed"
            )
            self.assertEqual(
                create_product_response.status_code, HTTPStatus.BAD_REQUEST
            )
    ...
    def test_create_product_invalid_xpath(self):
        with self.client:
            ...
            self.assertTrue(
                "XPath query must not be null or empty string.",
    ...
    def test_retrieve_product_does_not_exist(self):
        with self.client:
            ...
            self.assertTrue(
                retrieve_product_data["message"].startswith(
                    "python_v3_7 not found in database."
                )
            )
            self.assertEqual(retrieve_product_response.content_type, "application/json")
            self.assertEqual(
                retrieve_product_response.status_code, HTTPStatus.NOT_FOUND
            )
    ...
    def test_update_product(self):
        with self.client:
            ...
            self.assertEqual(
                update_product_data["xpath_version_number"], updated_xpath_1
            )
    ...
    def test_update_product_does_not_exist(self):
        with self.client:
            ...
            self.assertTrue(
                update_product_data["message"].startswith(
                    "Product name: python_v3_7 not found."
                )
            )
            self.assertEqual(update_product_response.content_type, "application/json")
    ...
    def test_delete_product(self):
        with self.client:
            ...
            delete_product_response = delete_product(
                self, product.product_name, jwt_auth
            )
    ...
    def test_delete_product_does_not_exist(self):
        with self.client:
            jwt_auth = create_admin_user_and_sign_in(self)
            delete_product_response = delete_product(self, "python_v3_7", jwt_auth)
            self.assertEqual(delete_product_response.content_type, "application/json")
            self.assertEqual(delete_product_response.status_code, HTTPStatus.NO_CONTENT)
        ...
        def test_delete_product_admin_token_requred(self):
            with self.client:
                ...
                self.assertEqual(
                    delete_product_response.content_type, "application/json"
                )
                self.assertEqual(
                    delete_product_response.status_code, HTTPStatus.UNAUTHORIZED
                )
